  ...
  return (
    <div className="pt-32">
      ...
      <section className="px-6 md:px-12 mb-24 max-w-[1440px] mx-auto">
        <AnimatedSection>
          <span className="font-label text-[#0145F2] font-semibold tracking-[0.3em] uppercase block mb-4">Social Responsibility</span>
          <h1 className="text-7xl md:text-9xl font-headline font-black tracking-tighter leading-none mb-8">
            Empowering <span className="text-[#0145F2] italic">Communities</span>
          </h1>
        ...
        <StaggerContainer className="max-w-[1440px] mx-auto grid grid-cols-1 md:grid-cols-4 gap-8 text-center">
          {[
            ...
            <StaggerItem key={s.label}>
              <div className="group">
                <motion.div whileHover={{ scale: 1.1 }} className="text-5xl md:text-6xl font-black font-headline text-on-surface group-hover:text-[#0145F2] transition-colors mb-2">{s.val}</motion.div>
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <StaggerItem key={item.title}>
                <motion.div whileHover={{ y: -8 }} className="glass-card p-10 rounded-xl border border-white/5 hover:border-[#0145F2]/30 transition-all duration-500 group h-full">
                  <div className="w-16 h-16 rounded-full bg-[#0145F2]/10 flex items-center justify-center mb-8 border border-[#0145F2]/30 group-hover:bg-[#0145F2] group-hover:text-[#FFFFFF] transition-all">
                    <span className="material-symbols-outlined text-3xl text-[#0145F2] group-hover:text-[#FFFFFF]">{item.icon}</span>
                  </div>
      ...
      <section className="py-32 px-6 md:px-12 bg-surface-container-low">
        <div className="max-w-[1440px] mx-auto grid lg:grid-cols-2 gap-20 items-center">
          <AnimatedSection direction="left">
            <span className="font-label text-[#0145F2] tracking-[0.3em] uppercase text-xs mb-4 block">The Shyam Foundation</span>
            <h2 className="text-5xl font-black font-headline tracking-tighter mb-8">Building Futures,<br /><span className="text-[#0145F2]">Beyond Metal.</span></h2>
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={item.title} className="flex items-start gap-4">
                  <span className="material-symbols-outlined text-[#0145F2] mt-1">check_circle</span>
                  <div>
          ...
          <AnimatedSection direction="right">
            <div className="relative">
              <div className="absolute -inset-10 bg-[#0145F2]/5 blur-[100px] rounded-full"></div>
      ...
      <section className="py-24 px-6 md:px-12 bg-surface text-center">
        <AnimatedSection>
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-slate-600 text-lg mb-10">Partner with the Shyam Foundation to create lasting impact in communities across India.</p>
            <motion.button whileHover={{ scale: 1.05, boxShadow: '0 0 30px rgba(1, 69, 242,0.3)' }} whileTap={{ scale: 0.95 }} className="bg-[#0145F2] text-[#FFFFFF] px-12 py-5 font-bold uppercase tracking-widest">Get Involved</motion.button>
          </div>
        </AnimatedSection>
      </section>
    </div>
