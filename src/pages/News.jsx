    ...
    <div className="pt-24">
      ...
      <section className="relative h-[870px] w-full overflow-hidden flex items-end">
        <div className="absolute inset-0 z-0">
          <img className="w-full h-full object-cover grayscale-[0.5] brightness-50" alt="dramatic wide shot of a modern steel foundry" src="https://lh3.googleusercontent.com/aida-public/AB6AXuAYRUmzexMwVohs8B2ZT65YqE2DXnO1-4oT0eMShaCU9uUMq1Kz66xDTy-q9jGmkbVSgUDT6aBFQgQOJ0TRimOt0j0TyfuicbGI4RNbxjyaNuzQvr8ITPEWaQUF6uaCOagCDRznTgKOIjSHQCGc-rnKC6fak7d-ukbkA4TmzS3wSfYAZuoXEIspGsgNfOy8-0vmGG9CF76bxttaWY5_c9Ma_6AYLS5DgihT_s3Y-f65P4h56zk3xxQf3XCcIgFFHekT6LqR9Nia2jY" />
          <div className="absolute inset-0 bg-gradient-to-t from-[#EDF1F5] via-transparent to-transparent"></div>
        </div>
        <div className="relative z-10 px-6 md:px-12 pb-24 max-w-[1920px] mx-auto w-full">
          <AnimatedSection>
            <div className="flex flex-col gap-6 max-w-4xl">
              <span className="inline-flex items-center gap-2 px-4 py-1 rounded-full bg-[#0145F2]/10 border border-[#0145F2]/20 text-[#0145F2] font-label text-xs tracking-widest uppercase">Featured Expansion</span>
      ...
      <section className="px-6 md:px-12 py-12 max-w-[1920px] mx-auto">
        <div className="flex flex-wrap items-center gap-4">
          {['ALL NEWS', 'CORPORATE', 'ESG & IMPACT', 'EXPANSION', 'FINANCIALS'].map((chip, i) => (
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} key={chip} className={`px-8 py-2 rounded-full font-label text-sm ${i === 0 ? 'bg-[#0145F2] text-[#FFFFFF] font-bold' : 'bg-surface-container-high text-on-surface-variant hover:text-on-background hover:border-[#0145F2]/30 border border-transparent'} transition-all`}>
              {chip}
            </motion.button>
          ))}
        </div>
              ...
              <motion.article whileHover={{ y: -8 }} className="group relative bg-surface-container-low overflow-hidden rounded-xl border border-white/5 transition-all duration-500">
                ...
                <div className="p-8">
                  <div className="flex justify-between items-center mb-4">
                    <span className="text-[#0145F2] font-label text-xs font-bold tracking-widest uppercase">{a.cat}</span>
                    <span className="text-slate-500 font-label text-xs">{a.date}</span>
                  </div>
                  <h3 className="text-2xl font-bold font-headline mb-4 leading-tight group-hover:text-[#0145F2] transition-colors">{a.title}</h3>
                  <p className="text-on-surface-variant text-sm mb-6 line-clamp-3">{a.desc}</p>
                  <span className="inline-flex items-center gap-2 text-[#0145F2] font-bold text-xs tracking-widest uppercase group-hover:gap-4 transition-all cursor-pointer">
      ...
      {/* Newsletter */}
      <section className="py-24 px-6 md:px-12 border-y border-[#0145F2]/10">
        <AnimatedSection>
          <div className="max-w-4xl mx-auto text-center">
            <span className="text-[#0145F2] font-label text-sm tracking-[0.3em] uppercase mb-4 block">Stay Informed</span>
            ...
            <form className="flex flex-col md:flex-row gap-4">
              <input className="flex-grow bg-surface-container-highest border-none focus:ring-2 focus:ring-[#0145F2] text-on-surface px-8 py-5 rounded-lg text-lg" placeholder="Your corporate email address" type="email" />
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="bg-[#0145F2] text-[#FFFFFF] font-black uppercase tracking-widest px-12 py-5 rounded-lg hover:shadow-[0_0_30px_rgba(1,69,242,0.3)] transition-all">JOIN NOW</motion.button>
            </form>
          </div>
        </AnimatedSection>
      </section>
