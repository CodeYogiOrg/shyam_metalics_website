...
export default function IronAndSteel() {
  return (
    <div className="pt-24">
      {/* Hero */}
      <WaveSection waves={4} className="relative min-h-screen flex items-center px-6 md:px-12 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl">
          <AnimatedSection delay={0.2}>
            <span className="font-label text-[#C3D809] tracking-[0.3em] uppercase text-sm mb-6 block">Core Division</span>
          </AnimatedSection>
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <StaggerItem key={p.title}>
                <motion.div whileHover={{ y: -8, borderColor: 'rgba(195,216,9,0.3)' }} className="glass-card p-10 rounded-xl border border-white/5 transition-all duration-500 group h-full">
                  <h3 className="text-2xl font-black mb-4 group-hover:text-[#C3D809] transition-colors">{p.title}</h3>
      ...
      {/* Stats */}
      <WaveSection waves={3} className="py-24 px-6 md:px-12 bg-surface">
          ...
          {[
            ...
            <StaggerItem key={s.label}>
              <div className="text-center group">
                <motion.div whileHover={{ scale: 1.1 }} className="text-5xl font-black font-headline text-on-surface group-hover:text-[#C3D809] transition-colors mb-2">{s.val}</motion.div>
      ...
      {/* CTA */}
      <section className="py-32 bg-[#1a181a] text-center relative overflow-hidden">
        <div className="relative z-10 px-6">
          <AnimatedSection>
            <h2 className="font-headline text-5xl md:text-7xl font-black tracking-tighter mb-10 text-zinc-100 uppercase">
              Build with the <br /> <span className="text-[#C3D809] text-glow">Best in Steel.</span>
            </h2>
          </AnimatedSection>
          <AnimatedSection delay={0.2}>
            <p className="max-w-2xl mx-auto text-zinc-500 mb-12 text-lg">Contact our industrial consultants for a personalized quote.</p>
          </AnimatedSection>
          <AnimatedSection delay={0.4}>
            <div className="flex flex-col sm:flex-row justify-center items-center gap-6">
              <motion.button whileHover={{ scale: 1.05, backgroundColor: '#C3D809', color: '#1a1c00' }} whileTap={{ scale: 0.95 }} className="px-10 py-5 bg-white text-black font-black uppercase tracking-widest text-sm transition-colors duration-300">Inquire Now</motion.button>
              <motion.button whileHover={{ scale: 1.05, borderColor: '#C3D809' }} whileTap={{ scale: 0.95 }} className="px-10 py-5 border border-zinc-700 text-white font-black uppercase tracking-widest text-sm transition-colors duration-300">Catalog 2024</motion.button>
            </div>
          </AnimatedSection>
        </div>
      </section>
