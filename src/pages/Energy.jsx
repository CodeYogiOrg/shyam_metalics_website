...
export default function Energy() {
  return (
    <div className="pt-32">
      {/* Hero */}
      <WaveSection waves={4} className="relative min-h-[70vh] flex items-center px-6 md:px-12 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <div className="absolute top-1/4 right-0 w-[600px] h-[600px] bg-[#C3D809]/10 blur-[150px] rounded-full"></div>
          <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-[#C3D809]/5 blur-[120px] rounded-full"></div>
        </div>
        <div className="max-w-[1920px] mx-auto w-full relative z-10">
          <AnimatedSection delay={0.2}>
            <span className="font-label text-[#C3D809] font-bold tracking-[0.3em] uppercase text-sm mb-6 block">Diversified Portfolio</span>
          </AnimatedSection>
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <StaggerItem key={item.title}>
                <motion.div whileHover={{ y: -8 }} className="glass-card p-12 rounded-xl border border-white/5 hover:border-[#C3D809]/30 transition-all duration-500 group h-full">
                  <div className="w-16 h-16 rounded-full bg-[#C3D809]/10 flex items-center justify-center mb-8 border border-[#C3D809]/30">
                    <span className="material-symbols-outlined text-3xl text-[#C3D809]">{item.icon}</span>
                  </div>
                  <div className="text-4xl font-black text-[#C3D809] mb-4">{item.stat}</div>
        ...
        <div className="max-w-[1920px] mx-auto">
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[
              ...
              <StaggerItem key={item.title}>
                <motion.div whileHover={{ y: -5, borderColor: 'rgba(195,216,9,0.15)' }} className="bg-surface-container-high p-10 rounded-xl transition-all duration-300 border border-transparent">
      ...
      <section className="py-32 px-6 md:px-12 bg-surface-container-lowest relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-[#C3D809]/30 to-transparent"></div>
        <AnimatedSection>
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-6">
              <motion.a whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="w-full md:w-auto px-12 py-5 molten-gradient text-[#1a1c00] font-headline font-extrabold uppercase tracking-widest text-sm" href="#">Partner With Us</motion.a>
